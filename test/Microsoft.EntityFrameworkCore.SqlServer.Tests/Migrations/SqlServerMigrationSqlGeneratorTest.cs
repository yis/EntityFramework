...
namespace Microsoft.EntityFrameworkCore.SqlServer.Tests.Migrations
{
    public class SqlServerMigrationSqlGeneratorTest : MigrationSqlGeneratorTestBase
    {
        ...
        public virtual void CreateDatabaseOperation_with_filename()
        {
            ...
            Assert.Equal(
                "CREATE DATABASE [Northwind]" + EOL +
                @"ON (NAME = 'Narf.mdf', FILENAME = 'C:\Narf.mdf')" + EOL +
                @"LOG ON (NAME = 'Narf_log.ldf', FILENAME = 'C:\Narf_log.ldf');" + EOL +
                "GO" + EOL +
                EOL +
                "IF SERVERPROPERTY('EngineEdition') <> 5 EXEC(N'ALTER DATABASE [Northwind] SET READ_COMMITTED_SNAPSHOT ON;');" + EOL,
                Sql);
        ...
        public virtual void CreateDatabaseOperation_with_filename_and_datadirectory()
        {
            ...
            var expectedLog = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Narf_log.ldf");

            Assert.Equal(
                "CREATE DATABASE [Northwind]" + EOL +
                "ON (NAME = 'Narf.mdf', FILENAME = '" + expectedFile + "')" + EOL +
                "LOG ON (NAME = 'Narf_log.ldf', FILENAME = '" + expectedLog + "');" + EOL +
                "GO" + EOL +
                EOL +
                "IF SERVERPROPERTY('EngineEdition') <> 5 EXEC(N'ALTER DATABASE [Northwind] SET READ_COMMITTED_SNAPSHOT ON;');" + EOL,
                Sql);
